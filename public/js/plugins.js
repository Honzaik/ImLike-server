/* 
	custom file for small plugins 
	-FontResize: resizes fonts based on the number of character in it, so it wont overlap the parent element
	-VertCent: vertically centers elements in the parent one
*/
...
VertCent.center = function(){
	...
	for(var i = 0; i < elementsToVertCent.length; i++){
		...
		var paddingBottom = parseInt(parent.css("paddingBottom"));
		var offset = 0;
		...
		if(elementH < parentH){
			var gap = ((parentH - offset - paddingTop - paddingBottom) - elementH)/2;
